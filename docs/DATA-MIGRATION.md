...
`견본 전보함 2` 전보함의 VOL.3~7 에는 발신인이 `"a"` / `"b"` 로만 적힌 더 옛 형식이
섞여 있다. `a` = 안, `b` = 에피 로 읽는다 (VOL.7 에서 같은 사람의 전보가
...
- 옛 `coupleCode`(`ABCD-2345` 꼴)를 새 초대 코드로 그대로 쓴다 — 네 사람이
  이미 외우고 있는 값이다. 형식이 안 맞으면 그때 결정한다.
- `견본 전보함 2` 는 `ownerUid` 가 빈 문자열이다. 안 를 소유자로 놓는다.
- 권 번호에 구멍이 있다 (`견본 전보함 3` 는 volSample, volSample / `견본 전보함 2` 는 volSample 부터).
  **메꾸지 않는다.** 옛 기록 그대로 둔다.
