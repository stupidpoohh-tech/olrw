...
| 가입은 `이미 가입된 이메일`, 로그인은 `비밀번호가 일치하지 않습니다` | 계정은 있는데 비밀번호를 모른다 | 로그인 화면의 **비밀번호를 잊으셨나요** |
| 재설정 메일이 안 온다 | Neon 쪽 메일 발송이 꺼져 있다 | 아래 **비밀번호를 잊었을 때** |
| `비밀번호는 맞았지만 이 브라우저에 로그인 상태가 남지 않습니다` | 브라우저가 쿠키를 막았고, 앱이 우회했는데도 안 됐다 | 아래 **로그인 상태가 저장되지 않을 때** |
| `계정은 만들었지만 이 브라우저에 로그인 상태가 남지 않습니다` | 같은 이유. 계정은 이미 만들어졌다 | 같은 절. 그 뒤에는 **로그인** 탭으로 들어갑니다 |
| 로그인 요청이 `/auth/…` 에서 404 | `public/_routes.json` 이 빠졌거나 함수가 배포되지 않았다 | 다시 배포합니다. `pnpm proxy:check` 로 함수를 먼저 확인합니다 |
| `로그인 정보를 저장하지 못했습니다` | 로그인 서버가 세션을 저장하지 못했다 | 잠시 뒤 다시. 계속되면 캡처해서 알려 주세요 |
...
증상은 이렇습니다. 가입을 누르면 오류가 뜨는데 **계정은 만들어져 있고**(다시
가입하면 "이미 가입된 이메일"), 로그인을 눌러도 같은 자리에서 막힙니다.
새로고침해도 그대로입니다.

비밀번호 문제가 아닙니다. Neon 의 로그인 서버는 이 앱과 **다른 사이트**
(`*.neon.tech`)에 있고, 로그인한 상태는 그 사이트의 쿠키로 유지됩니다. 사파리는
크로스 사이트 추적 방지를 기본으로 켜 두고 그런 쿠키를 막습니다 — 로그인 자체는
통과해도 그다음 확인이 늘 빈손으로 돌아옵니다.

**앱이 알아서 처리합니다.** 세션이 남지 않은 것을 확인하면 그 자리에서 로그인만
우리 주소 밑(`olrw-8pt.pages.dev/auth/…`)으로 돌려 한 번 더 시도합니다. 그러면
브라우저가 보는 상대가 우리 도메인 하나뿐이라 추적 방지와 무관해집니다. 그 브라우저는
다음 방문부터 처음부터 그 길로 갑니다. 설정을 바꿀 필요가 없습니다.

- 중계하는 것은 `functions/auth/[[path]].js` 이고, `/auth/*` 하나만 탑니다
  (`public/_routes.json`). 나머지 주소는 전부 예전 그대로 정적 파일입니다
- 전보를 읽고 쓰는 쪽(Data API)은 쿠키가 아니라 토큰으로 다니므로 손대지 않았습니다
- 잘 되던 분들의 경로는 바뀌지 않습니다. 실패한 그 사람만 갈아탑니다

그 길로도 안 되면 화면에 그대로 알립니다. 그때는 화면을 캡처해서 알려 주십시오.
오류 문장 아래 작은 글씨로 표식이 붙어 있으면 함께 보내 주시면 자리를 바로 알 수
있습니다.

### 비밀번호를 잊었을 때

로그인 화면 아래 **비밀번호를 잊으셨나요** → 가입한 이메일을 넣고 **재설정 메일
보내기**. 메일의 링크를 누르면 새 비밀번호를 정하는 화면이 열립니다.

- 링크는 **한 번만** 통합니다. 다시 필요하면 다시 요청합니다
- 가입한 적 없는 주소를 넣어도 같은 화면이 뜹니다 — 여기서 "그런 계정 없습니다"
  를 알려 주면 누구나 이메일을 넣어 보며 가입 여부를 캘 수 있습니다
- 링크가 돌아오는 주소(`https://olrw-8pt.pages.dev`)가 **§5-1 의 Domains 에
  있어야** 합니다. 없으면 서버가 링크를 거릅니다

#### 메일이 오지 않으면

Neon 이 재설정 메일을 보내도록 켜져 있어야 합니다.

Neon 콘솔 → 왼쪽 메뉴 **Auth** → 위쪽 **Configuration** 탭 → **Emails**
(또는 **Email templates**) 에서 비밀번호 재설정 메일이 켜져 있는지 봅니다.

그래도 안 오면 방장이 계정을 지워 주는 길이 남아 있습니다.

Neon 콘솔 → 왼쪽 메뉴 **Auth** → 위쪽 **Users** 탭 → 이메일로 그 사람을 찾아
오른쪽 **⋯** → **Delete user**

- **아직 어느 전보함에도 들어가지 않았다면** 잃는 것이 없습니다. 다시 가입하면 됩니다.
- **이미 전보함에 들어가 있다면 지우지 마십시오.** 그 사람의 uuid 가 `box_members` 와
  `volume_pages` 에 박혀 있어, 계정을 지우고 새로 만들면 uuid 가 달라져
  전보함에서 빠집니다. 그때는 물어봐 주십시오 — SQL 로 이어 붙입니다.
