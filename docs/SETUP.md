...
| `비밀번호는 맞았지만 이 브라우저가 로그인 상태를 저장하지 못했습니다` | 브라우저가 크로스 사이트 쿠키를 막고 있다 | 아래 **로그인 상태가 저장되지 않을 때** |
| `계정은 만들었지만 이 브라우저가 로그인 상태를 저장하지 못했습니다` | 같은 이유. 계정은 이미 만들어졌다 | 같은 절. 그 뒤에는 **로그인** 탭으로 들어갑니다 |
...
비밀번호 문제가 아닙니다. 로그인 서버는 이 앱과 **다른 사이트**(`*.neon.tech`)에
있고, 로그인한 상태는 그 사이트의 쿠키로 유지됩니다. 브라우저가 다른 사이트의
쿠키를 막으면 로그인 자체는 통과해도 그다음 확인이 늘 빈손으로 돌아옵니다.

먼저 해 볼 것은 순서대로 이렇습니다.

1. **다른 브라우저로 열어 봅니다.** 크롬에서 되면 원인이 확정됩니다.
2. **사생활 보호 창(시크릿)이면 닫고** 보통 창에서 다시 엽니다.
3. 아이폰 사파리라면 — 설정 앱 → **앱** → **Safari** →
   **크로스 사이트 추적 방지** 를 끄고 다시 시도합니다.
   (안드로이드 크롬은 설정 → **개인정보 보호 및 보안** → **서드 파티 쿠키**)

계정은 이미 만들어져 있으므로, 위 셋 중 하나로 통하면 **가입이 아니라 로그인**
탭으로 들어갑니다.

세 가지 다 해 봤는데도 같으면 화면을 캡처해서 알려 주십시오. 오류 문장 아래에
작은 글씨로 표식이 붙어 있으면 그것까지 함께 보내 주시면 어느 자리인지 바로
알 수 있습니다.
