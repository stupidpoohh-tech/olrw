...
## 5-1. 앱 주소를 로그인 서버에 등록합니다 (빠뜨리면 로그인이 안 됩니다)

**이 단계를 건너뛰면 배포한 앱에서 로그인 버튼이 아무 일도 하지 않습니다.**
개발자 도구 Console 에 `403` 이 여러 줄 뜨고, 화면에는

> 이 주소는 로그인 서버에 등록되어 있지 않습니다 (https://…). Neon 콘솔 →
> Auth → Configuration → Domains 에 이 주소를 넣어야 합니다.

가 뜹니다. 비밀번호가 틀린 것이 아닙니다 — 로그인 서버가 **어느 주소에서 온
요청인지**를 먼저 보고, 등록되지 않은 주소면 비밀번호를 보기도 전에 끊습니다.

### 넣는 곳

Neon 콘솔 → 왼쪽 메뉴 **Auth** → 위쪽 **Configuration** 탭 → **Domains**
→ **Add domain**
...
| 무엇 | 값 |
|---|---|
| 배포 주소 | `https://olrw-8pt.pages.dev` |
| 미리보기 주소 전부 | `https://*.olrw-8pt.pages.dev` |
| 내 컴퓨터에서 개발할 때 | `http://localhost:5173` |

- **`https://` 를 포함해서** 넣습니다. 주소만 넣으면 안 맞습니다
- **끝에 `/` 를 붙이지 않습니다**
- 한 줄에 하나씩, 세 개를 각각 **Add domain** 합니다

두 번째 줄의 `*` 는 미리보기 주소를 한꺼번에 받는 자리표입니다. Cloudflare 는
가지를 새로 밀 때마다 `<가지이름>.olrw-8pt.pages.dev` 처럼 **매번 다른 주소**를
내주므로, 하나씩 등록하면 가지를 만들 때마다 이 화면에 다시 와야 합니다.

### §8 의 CORS 와 다른 설정입니다

이름이 비슷해 헷갈리지만 **둘은 다른 화면이고 서로를 대신하지 못합니다.**

| | 어디 | 무엇을 지키나 |
|---|---|---|
| 여기 (§5-1) | **Auth** → Configuration → Domains | **로그인·가입** 요청 |
| §8 | **Postgres database** → Data API → Settings | **전보·전보함 조회** 요청 |

로그인이 403 이면 여기(§5-1)를, 로그인은 되는데 전보함이 안 불러와지면 §8 을 봅니다.
...
**이것은 로그인과 무관합니다.** 로그인이 막히면 §5-1 을 봅니다 — 그건 **Auth**
페이지의 Domains 이고, 여기는 **Data API** 페이지의 CORS 입니다.
...
| 증상 | 원인 | 할 일 |
|---|---|---|
| **로그인·가입을 눌러도 아무 일이 없다.** Console 에 `403` 이 여러 줄 | 앱 주소가 로그인 서버에 등록되지 않았다 (`Invalid origin`) | **§5-1** — Auth → Configuration → Domains |
...
| 로그인은 되는데 전보함이 안 불러와진다 | Data API 의 CORS 출처에 그 주소가 없다 | §8 |
| 새 가지의 미리보기 주소에서만 로그인이 막힌다 | Domains 에 `*` 줄이 없다 | §5-1 — `https://*.olrw-8pt.pages.dev` |
