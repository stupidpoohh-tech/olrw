...
| 가입은 `이미 가입된 이메일`, 로그인은 `비밀번호가 일치하지 않습니다` | 계정은 있는데 비밀번호를 모른다 | 아래 **비밀번호를 잊었을 때** |
...
**앱에는 비밀번호 재설정이 아직 없습니다.** 가입 탭에서는 "이미 가입된 이메일",
로그인 탭에서는 "비밀번호가 일치하지 않습니다" 가 뜨면 그 계정으로는 들어갈 길이
없습니다. 방장이 계정을 지워 주면 같은 이메일로 다시 가입할 수 있습니다.
...
지운 뒤 다시 가입할 때 **비밀번호는 8자 이상**입니다. 앱이 8자 미만은 아예 못
넣게 막지만, 옛 화면(6자)을 보고 있다면 새로고침을 한 번 해 주십시오.
