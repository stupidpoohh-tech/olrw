...
/**
 * 녹음이 실제로 들어왔는지 밖에서 확인할 수 있게 열어 둔다.
 *
 * 오디오는 조용히 망가진다 — 받아오지 못하면 합성으로 떨어지고 화면은 멀쩡하다.
 * 그걸 소리의 **길이로 어림잡던** 시험이 있었는데, 여섯 벌의 파형 모양이 서로
 * 달라 피크가 두 배까지 벌어지고 판정이 흔들렸다(체감 크기 RMS 는 고르다).
 * 버퍼를 직접 보면 흔들릴 것이 없다. `pnpm sound:check` 가 이 문으로 확인한다.
 */
export { keyBuffer } from './keySamples';
